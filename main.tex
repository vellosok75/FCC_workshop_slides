...
\begin{frame}[plain]
    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        % Author
        \node[below=3cm] (author) at (title.south){Matheus M. S. Velloso};

        % Date
        \node[below=0.25cm] (date) at (author.south){\large \today};
...
\begin{frame}{RCDS optimization setup}
    \begin{minipage}{0.49\textwidth}
        Obj. func: avg. injection efficiency of 5 pulses @ $\unit{2~Hz}$\\
        Knobs: 21 sextupole families
        \begin{table}[]
            \begin{tabular}{cl}
            \hline
    ...
    \end{minipage}
    \begin{minipage}{0.49\textwidth}
        Changing sextupoles w/o changing chromaticity:
        \begin{itemize}
            \item knobs $\in$ chromaticity response matrix nullspace
            \item free knobs + compensation knobs
        \end{itemize}
        \vspace{0.5cm}
